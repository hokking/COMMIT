...
<div class="side-menu-button">
  ...
  <div class="side-menu-bar">
   ...
   <input id = "sbar3" type = "submit" style='cursor:pointer' value="">
      <button style='cursor:pointer' type="button" onclick="location.href='<%=request.getContextPath()%>/board/login.jsp'">
...
<section>
	...
	int rs = service.selectrecruit(memid);
	
	List<RecruitVO> list = service.listrecruit(memid);
	%> 
	...
	if(rs==0){ //이력서 없으면
	%>
	등록된 공고가 없습니다. 공고를 등록해주세요<br><br>
	<button id="bresume" style='cursor:pointer' onclick="location.href='<%=request.getContextPath() %>/mypage/'" type="button" >
	공고 등록하기</button>
	

	<%
	}else if(rs!=0){ // 이력서 있으면
		
		
	%>
	
	<br><br>
	<div id="cnt">총 <%=rs%>건</div>
	
	<button id="bresume" style='cursor:pointer' onclick="location.href='<%=request.getContextPath() %>/mypage/.jsp'" type="button" >
	공고 등록하기</button>
	...
	<div id="title">
		 <a href="<%=request.getContextPath() %>/mypage/resumeD.jsp?resume_num=<%= vo.getRecruit_num()%>"><%=vo.getRecruit_name()%>
		 </a>
	</div>
	<br>
